...
gh release create "$RELEASE_TAG" --repo ATIinc/onnxruntime --target "$RELEASE_BRANCH" \
	...
	--notes "Upstream v1.26.0 rebuilt with CUDA kernels for sm_75, sm_86, sm_89 and sm_120 (plus sm_120 PTX) so Blackwell GPUs such as the RTX 5060 Ti do not JIT every kernel on first inference. Built with ati/build-wheel.sh (CUDA 12.8, cuDNN 9, Python 3.12, x86_64 Linux)." \
	"$wheel"
